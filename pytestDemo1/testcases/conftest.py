...
def get_basedata_url(sql):
    for sq in range(1, str(sql).count('${') + 1):
        if '${' in sql and '}' in sql:
            ...
            endIndex = str(sql).find('}')
            oldValue = str(sql)[int(startIndex):int(endIndex) + 1]
            newValue = ReadFileData().read_extract_yaml(str(oldValue)[2:-1])
            sql = str(sql).replace(oldValue, newValue)
    return sql
...
def select_user_order():
    ...
    for dis in user_distributor_id:
        ...
        logger.info("订单创建成功，通过订单编号查到的distributor_id是：{}".format(dis['user_distributor_id']))
        logger.info("订单创建成功，登录的distributor_id是：{}".format(ReadFileData().read_extract_yaml("distributor_id")))
    logger.info("执行后置sql:{}".format(select_sql))
